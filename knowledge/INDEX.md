...
- [0050-rtfm-hierarchy](experiments/0050-rtfm-hierarchy.md) — PRE-REGISTERED build (maintainer chose the full hierarchy): Director-style manager/worker in imagination to break the ~0.10 execution ceiling. Phase A = hierarchy WITHOUT manual (isolate credit-assignment vs grounding); Phase B = manual-DIRECTED manager (the manual IS the decomposition tree). Stability via HAC hindsight-relabel + codebook pre-seeded from VR checkpoints. Lit: [[director-2022]]/FuN/HAC/THICK. Multi-session build (planned)
