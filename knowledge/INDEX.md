...
- [0050-rtfm-hierarchy](experiments/0050-rtfm-hierarchy.md) — Director manager/worker hierarchy to break the ~0.10 execution ceiling. STALLED (2 attempts, headline stuck at ~0): v1 diverged (worker_loss ±80 + collection-poisoning); v2 (advantage-norm + grad-clip + flat-collect) stable but correct/swap_follow 0.00 across 4 seeds even over a good WM. Diagnosis: the minimal cut (raw belief-space goals + K-means + cosine, NO goal autoencoder) — Director's ablation says the autoencoder is load-bearing. Handed back: (A) VQ-VAE goal autoencoder / (B) recursive MPC fallback / (C) bank VR (verified)
