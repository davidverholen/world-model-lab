...
- [0048-rtfm-validated-reading](experiments/0048-rtfm-validated-reading.md) — intrinsic "validated-reading" reward (manual's marginal next-state predictive value, confirmed by the REAL transition; deter_noctx baseline; reality = judge → anti-baking). PHASE-1 POSITIVE (1 seed): FIRST lever to move length-2 swap_follow off 0 → 0.16–0.18 at α=0.1–0.3 (swapped=0), events 2.8→7.4, oracle_pct→0.92 (beat the 0044–0047 thread), clean inverted-U (α=1.0 dark-rooms). 4-seed control running; candidate until it confirms (draft)
- [0049-rtfm-sustained-vr](experiments/0049-rtfm-sustained-vr.md) — follow-up: push past the ~0.18 swap_follow plateau via the α-ridge (0.2–0.5, below the 1.0 dark-room) + a reading-shaping FLOOR (don't fully anneal, so VR replaces the scaffold). Contingent on the 0048 control. Bar: swap_follow >0.25 sustained, multi-seed (planned)
