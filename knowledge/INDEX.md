...
- [0057-rtfm-path-reward](experiments/0057-rtfm-path-reward.md) — REWARD THE PATH: the env's reading_shaping is already a per-step gesture reward but we anneal it to 0 exactly during length-2. Re-analysis of exp0049 (floor 0.2) via swap_follow_s1: keeping it lifted step-1 reading 0.28→0.42 but NOT conditional step-2 (~0.17) → uniform path reward helps getting ON the path, not step-2's conditional reliability (exposure-starved?). Sweeping floor {0.5,1.0}. IN FLIGHT (draft)
- [0058-rtfm-escalating-path-reward](experiments/0058-rtfm-escalating-path-reward.md) — ESCALATING per-step path reward (reward step-k with coef·factor^k): targets exp0057's finding that step-2 contributes ~6× less than step-1 to a uniform reward (actor satisfices on step-1). Sweeps factor {3,5,10}. Tests reward-magnitude vs step-2 exposure. BUILT + reviewer-SHIP; dispatch when GPU frees (draft)
