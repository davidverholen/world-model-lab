...
- [rtfm-rollout-perf](design/rtfm-rollout-perf.md) — rung-4 collection is bound by crafter WORLD-GEN (~79%, simplex noise at reset), not GPU/encode; vectorizing the encode REFUTED (~0 win, reverted); real levers = process-parallelism (now) + world-gen caching (handoff) (verified)
...
- [0054-rtfm-reward-gap](experiments/0054-rtfm-reward-gap.md) — reward-gap probe on the working baseline (folded VR 0.3, rew head saved). CONCLUDED: reward head is OVER-OPTIMISTIC both seeds (imagined−real reward +0.67/+0.79 over 8 steps, real≈0 → mostly phantom) atop FAITHFUL dynamics. The ~0.10 wall has a confirmed reward-READOUT component (exp0045 ghost on the working baseline). Next lever = CALIBRATED conservatism (exp0047 blunt push-down over-suppressed); residual = credit-assignment/execution (exp0043) (verified)
- [0055-rtfm-ensemble-pessimism](experiments/0055-rtfm-ensemble-pessimism.md) — ensemble-uncertainty (MOPO) pessimism (K=5, actor optimises mean−λ·std). CONCLUDED counter-outcome (λ∈{0.5..5} swap_follow 0.05–0.08, no break; reward head still over-optimistic) — but PIVOTAL: the new per-step metric showed reading WORKS (step-1 swap_follow ~0.4) and the wall is 2-STEP COMPOSITION (full chain ~0.08). Relabels the '~0.10 grounding ceiling' as a depth-2 sequencing wall (verified)
- [0056-rtfm-coverage-sweep](experiments/0056-rtfm-coverage-sweep.md) — coverage sweep (--n-train-seeds 100/400/1600) for the depth-2 composition wall. DONE (1 seed): PROMISING — 4× more distinct recipes (n1600) ~tripled conditional step-2 (P(s2|s1) 0.14→0.40) and ~2.4× exact (0.046→0.112), step-1 flat. Depth-2 wall is partly COVERAGE-bound (combinatorial), not just compute (exp0051 2× budget was flat). Needs multi-seed confirm (verified)
- [0057-rtfm-path-reward](experiments/0057-rtfm-path-reward.md) — REWARD THE PATH (env reading_shaping is a per-step gesture reward we anneal off). DONE (1 seed): floor 0.5 LIFTS conditional step-2 to 0.38 (from ~0.17) + exact to 0.14 — first path reward to move the conditional; floor 1.0 over-shapes (back to 0.18) → magnitude SWEET SPOT. CONVERGES with coverage n1600 (~0.38-0.40 cond both) → depth-2 wall is movable. Needs multi-seed (verified)
- [0058-rtfm-escalating-path-reward](experiments/0058-rtfm-escalating-path-reward.md) — escalating per-step path reward (coef·factor^k·decay^n). DONE: CONFOUNDED NEGATIVE — base coef 0.2 starved step-1 (collapsed to 0.13-0.17 vs 0.28; exp0057 showed step-1 needs ~0.5), so below baseline. NOT a clean back-loading test. Lesson: step-1 magnitude is load-bearing. Fix deferred (re-run base 0.5); prioritised floor-0.5 multiseed confirm + coverage×floor combo instead (verified)
- [0059-rtfm-floor-confirm](experiments/0059-rtfm-floor-confirm.md) — 4-seed multi-seed confirm of floor-0.5 path reward: robustly lifts step-1 (0.28→0.36) and conditional (0.17→0.285) across all seeds; exact lands ~0.103 (historical ceiling, not broken); single-seed exp0057 was optimistic (verified)
...
- [0062-rtfm-coverage-confirm](experiments/0062-rtfm-coverage-confirm.md) — coverage n1600 × uniform-0.5, 4-seed confirm. DONE: the single-seed coverage win did NOT replicate — n1600 4-seed exact 0.057 < n400 0.115 (cond 0.16 vs 0.33), step-1 unchanged. BUT confounded: n1600@30r sees each recipe only ~1.1x (vs n400 4.5x) → per-recipe under-trained, not 'coverage is bad'. Motivates the exp0063 long run (verified)
- [0063-rtfm-long-run-ceiling](experiments/0063-rtfm-long-run-ceiling.md) — LONGER run (20 curriculum + 60 length-2 = 80 rounds) of uniform-0.5 path reward at HIGH COVERAGE (n1600, generalization focus), to test whether the still-rising-at-r30 trend breaks the ~0.10 ceiling. Combines both step-2 levers + training length; exp0062 (n1600, 30r) is the clean 30-round reference. PRE-REGISTERED, NOT YET RUN — queued for next session (draft)
